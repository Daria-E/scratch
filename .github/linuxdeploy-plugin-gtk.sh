...
# drop bundled wayland libs: host mesa must pair with host wayland (EGL_BAD_PARAMETER otherwise)
rm -f "$APPDIR"/usr/lib/libwayland-*.so* "$APPDIR"/usr/lib/im-wayland*.so
